...
const Home = ({ isDark }) => {
  ...
  return (
    <>
      <div className="my-5">
        <h1 className={`text-2xl font-bold mb-4 text-center ${isDark ? "text-white" : "text-black"}`}>
          CodeRoaster AI 🔥
        </h1>
        
        <p className={`mb-4 text-center ${isDark ? "text-gray-300" : "text-black"}`}>
        ...
        <div className="flex flex-col gap-4 items-center justify-center w-full px-4">
          ...
          {roast && (
            <div className="mt-6 p-4 w-full max-w-lg bg-red-50 border border-red-200 rounded-lg dark:bg-red-900/30 dark:border-red-800">
              ...
              <h2 className={`text-xl font-bold mb-2 ${isDark ? "text-violet-300" : "text-violet-900"}`}>
                🔥 The Verdict:
              </h2>
              
              {/* Force the Markdown text color */}
              <div className={`whitespace-pre-wrap leading-relaxed font-medium [&>*]:text-current ${isDark ? "text-gray-100" : "text-black"}`}>
